...
async function bulkDeleteLocations(
  req: NextApiRequest,
  res: NextApiResponse<BulkDeleteResponse>
) {
  try {
...
async function bulkCreateLocations(
  req: NextApiRequest,
  res: NextApiResponse<BulkCreateResponse>
) {
  try {
    ...
    const newLocations = validLocations.filter(loc => 
      !existingSet.has(`${loc.detail.toLowerCase()}-${loc.province}`)
    )
